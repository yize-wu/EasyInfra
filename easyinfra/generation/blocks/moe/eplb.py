...
def balanced_packing(weight: torch.Tensor, num_packs: int, enable_eb2: bool) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    ...
    cumulated_pack_weights = [0] * num_packs
    for i in range(num_layers):
        ...
        for group in indices[i]:
            ...
            pack_items[pack] += 1
            cumulated_pack_weights[pack] += weight[i, group]
        # print(f"layer {i} pack weights: {pack_weights}")
        max_weight = max(cumulated_pack_weights)
        mean_weight = sum(cumulated_pack_weights) / num_packs
        print(f"layer {i} pack weights: {max_weight}/{mean_weight}={max_weight / mean_weight}")

    max_weight = max(cumulated_pack_weights)
    mean_weight = sum(cumulated_pack_weights) / num_packs
    print(f"final pack weights: {max_weight}/{mean_weight}={max_weight / mean_weight}")
    return pack_index, rank_in_pack
...
def rebalance_experts_hierarchical(weight: torch.Tensor, num_physical_experts: int, 
    ...
    group_pack_index, group_rank_in_pack = balanced_packing(tokens_per_group, num_nodes, enable_eb2) 
    ...
    pack_index, rank_in_pack = balanced_packing(tokens_per_phy, num_gpus // num_nodes, enable_eb2)
...
def rebalance_experts(weight: torch.Tensor, num_replicas: int, num_groups: int,
                      ...
                      enable_eb2: bool = False) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    ...
    if num_groups % num_nodes == 0:
        ...
        phy2log, phyrank, logcnt = rebalance_experts_hierarchical(weight, num_replicas, 1, 1, num_gpus, enable_eb2)
